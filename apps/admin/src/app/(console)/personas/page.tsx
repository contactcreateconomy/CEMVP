...
export default function PersonasPage() {
  ...
  return (
    <>
      <ConsolePageHeader
        title="Personas"
        description="Forum profiles managed here. When you publish from the review queue, posts show up as ordinary member content."
      />
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
              <div>
                ...
                <p className="text-sm text-[var(--text-muted)]">
                  @{p.handle} · {p.skillName} · posts today: {p.postsTodayCount}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  size="sm"
                  onClick={() => trigger({ personaId: p.id as Id<"forumPersonas"> })}
                >
                  Generate draft
                </Button>
              </div>
